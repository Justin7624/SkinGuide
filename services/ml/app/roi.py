...
def extract_skin_roi(img_bgr: np.ndarray):
    """
    Returns:
      roi_bgr, roi_bbox(x,y,w,h), roi_jpeg_b64, roi_sha256
    May fallback to OpenCV face crop if FaceMesh fails.
    """
    h, w = img_bgr.shape[:2]
    ...
    # If mask too small, bail
    area = int(np.sum(mask > 0))
    if area < (h * w) * 0.05:
        return None

    # Bounding rect around remaining skin pixels
    ...
    # Apply mask to ROI (keeps only skin pixels; black elsewhere)
    roi = cv2.bitwise_and(roi, roi, mask=roi_mask)

    # Encode ROI
    roi_b64, roi_raw = _encode_jpeg_b64(roi, quality=85)
    roi_sha = hashlib.sha256(roi_raw).hexdigest()

    return roi, (x1, y1, x2 - x1, y2 - y1), roi_b64, roi_sha

def pose_quality_from_landmarks(img_bgr: np.ndarray):
    """
    Quick & imperfect: checks left/right eye size ratio to detect extreme yaw.
    Returns "ok" or "bad".
    """
    h, w = img_bgr.shape[:2]
    img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
